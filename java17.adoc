...
node "Module recovery" {
	...
	usecasePremiereRelance - Invoice
}
...
node "Domain model" {
	...
	class Invoice implements AmountCalculable, Envoyable {
	Integer calculateRemainingAmountToPay()
	List<Personne> getRecipients()
  }

  interface AmountCalculable
  interface Envoyable
}
...
class Invoice implements AmountCalculable, Envoyable {
	...
	@Override
	public List<Personne> getRecipients(){
		...
	}
...
Le module letter utilise `Invoice` et se retrouve à pouvoir appeler les méthodes du module d’envoi ; ce qui viole au moins :

* Le https://fr.wikipedia.org/wiki/Principe_de_s%C3%A9gr%C3%A9gation_des_interfaces[principe de ségrégation des interfaces]
* Le https://fr.wikipedia.org/wiki/Principe_de_responsabilit%C3%A9_unique[principe de responsabilité unique] (Car la classe Document a maintenant 2 raisons de changer : le contexte _letter_ et le contexte _envoi_)

Effet bonus : Quand on change `Invoice` dans le cadre du contexte _letter_, on doit recompiler/relivrer aussi le contexte _envoi_.
...
[source,java]
----
if (invoice instanceof PaidInvoice) { // Oui je mets des accents dans mon code français. La sémantique !
	letterService.letter(((PaidInvoice) invoice));
	return;
}
if (invoice instanceof DueInvoice) {
	recoveryService.relancer(((DueInvoice) invoice));
}
----
...
if (invoice instanceof DueInvoice dueInvoice) {
	recoveryService.relancer(dueInvoice);
}
----
...
interface RecoveryService {
	void relancer(DueInvoice dueInvoice);
}
...
record InvoiceService(MainInvoiceVisitor mainInvoiceVisitor) implements InvoiceProcessing {
	...
	public void traiterInvoice(Invoice invoice) {
...
record InvoiceService(LetterService letterService, RecoveryService recoveryService) implements InvoiceProcessing {
	...
	public void traiterInvoice(Invoice invoice) {
		switch (invoice) {
			...
			// case DueInvoice dueInvoice -> recoveryService.relancer(dueInvoice);
		}
	}
}
----
...
record InvoiceService(
	...
	public void traiterInvoice(Invoice invoice) {
		switch (invoice) {
			case PaidInvoice paidInvoice -> letterService.letter(paidInvoice);
			case DueInvoice dueInvoice && dueInvoice.aDéjàÉtéRelancée() -> recoveryService.demarrerRecovery(dueInvoice);
			case DueInvoice dueInvoice -> recoveryService.relancer(dueInvoice);
		}
	}
}
...
[source,java]
----
record Invoice(String code, String libellé, Integer amount, ZonedDateTime dateCréation, ...){}
----
...
class InvoiceControlleur {
	...
	public Long create(@RequestBody Invoice invoice) {
		...
		notifierNouvelleInvoiceUseCase.handle(invoice);
...
* Un boucle de feedback plus rapide
